...
[原题链接](https://leetcode-cn.com/contest/biweekly-contest-9/problems/minimum-time-to-build-blocks/)
...
1. 将 `blocks` 转为堆结构
2. 每次取出堆中最小的两个元素 `b1` 和 `b2`
3. 计算 `split + max(b1, b2)`，表示**复制一个工人的耗时 + 并行修两个街区的最大耗时**，并加入堆
